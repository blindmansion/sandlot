...
- `render(entryPoint, options?)` — bundle and mount into the visible iframe, which provides a single `<div id="root">` host element; returns `{ ok, log, error? }`. Entry points should mount their UI into `#root` (e.g. `document.getElementById("root")`). `options.css` supplies an initial CSS override that replaces the bundle's extracted CSS (and persists as the override for later `updateCss()` calls).
- `updateCss(css?)` — hot-swap the CSS of the active render **in place** — no document reload, no JS re-execution, zero DOM/state loss. With no argument, re-bundles the active entry and swaps in the freshly extracted CSS (the natural flow after editing an imported `.css` file); an explicit `css` string overrides the bundle output and becomes the new override for later calls. Returns `{ ok, css? }`. Requires a prior `render(...)`.
- `hotUpdate()` — apply source edits made since the last render/update to the live render **without a full reload**, preserving the iframe realm, `window`, and CSS. Changed modules are re-compiled and hot-swapped, running the accept-boundary walk: a module that opted in via `import.meta.hot.accept()` re-runs only its own subgraph (`mode: "boundary"`, sibling component state preserved); otherwise the entry is re-run in place (`mode: "rerun"`, in-app state resets). Structural changes (installs, manifest edits, deletions) and unresolvable patches fall back to a fresh mount. Returns `{ ok, outcome: "patched" | "reloaded" | "noop" | "error", modules?, mode?, boundaries? }`. Requires a prior `render(...)`.
...
- `fixtures()` — list committed fixtures under `test/fixtures/` (currently `app-with-dep`, `basic`, `lit-app`, `notes-app`, `profiling`, `react-app`).
...
- `reset()` — clear the in-memory filesystem (including installed packages), reset the typecheck/bundle sessions, and tear down the current render (blanking the iframe and invalidating its handles), to switch tasks without reloading the page. Returns `{ removed }`, the number of top-level entries cleared.
